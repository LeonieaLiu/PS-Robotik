...
def maze_wall(tags):
    ...
    for tag in tags:
        ...
        if coord not in wall_set:
            wall_set.add(coord)
        if coord_2 not in wall_set_2:
            wall_set_2.add(coord_2)
    grid = np.ones((max_x, max_y))
    return wall_set, wall_set_2, grid
...
def path_to_movements(path):
    ...
    for i  in range(len(path) - 1):
        ...
        if movement_list[i][1] < 0:
            movement_list[i][2] = 180
        else:
            movement_list[i][2] = -90 * movement_list[i][0]
