...
def prevent_wall(subscriber, publisher, movements, i):
    x_old, y_old, uselesstuple_1 = findself(subscriber)
    rospy.sleep(0.3)
    orientation = subscriber.get_orientation()
    x_now, y_now, uselesstuple_2 = findself(subscriber)
    if np.abs(orientation) <= 45 or 135 <= np.abs(orientation) <= 180:
        if np.abs(y_old - y_now) < 0.03:
            publisher.stop()
            rospy.loginfo("Jetbot stopped due to some un expected reasons.")
            publisher.backward_slow()
            rospy.sleep(0.2)
            publisher.stop()
            pose_calib(publisher, subscriber, movements, i)
            publisher.forward_slow()
            rospy.sleep(0.2)
            pose_calib(publisher, subscriber, movements, i)

    elif 45 < np.abs(orientation) < 135:
        if np.abs(x_old - x_now) < 0.03:
            rospy.loginfo("Jetbot stopped due to some un expected reasons.")
            publisher.backward_slow()
            rospy.sleep(0.2)
            publisher.stop()
...
#            print("posx=", pos_x, "posy=", pos_y, "i=", i, "length=", len(path))
            ...
            if i < 1 or movement_list[i][2] - movement_list[i-1][2] == 0 and i != len(path) - 1:
                publisher.forward()
                prevent_wall(subscriber, publisher, movement_list, i)
            ...
            if movement_list[i][2] - movement_list[i-1][2] != 0 and i >= 1 and i != len(path) - 1:
                ...
                while np.abs(imu_diff) >= 6:
                    if 6 < imu_diff <= 180 or imu_diff < -180:
                        publisher.stay_turn_left_slow()
#                      rospy.loginfo("Turning left.")
                    if -180 <= imu_diff < -6 or 180 < imu_diff:
                        publisher.stay_turn_right_slow()
#                    orientation_2 = subscriber.get_rotation_matrix()
                    current_imu = subscriber.get_imu_degree()
                    imu_diff = target_imu - current_imu
